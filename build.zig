...
pub fn build(b: *std.Build) void {
    ...
    const use_c = b.option(bool, "use-c", "Build C versions of the library without exception support") orelse false;
    ...
    var mp_mprompt_name: [:0]const u8 = "mprompt";
    var mp_mpeff_name: [:0]const u8 = "mpeff";

    if (!use_c) {
        std.debug.print("Use the C++ compiler to compile\n", .{});
        mp_mprompt_name = "mpromptx";
        mp_mpeff_name = "mpeffx";
    } else {
        std.debug.print("Use the C compiler to compile (MP_USE_C=ON)\n", .{});
    }
    ...
    if (optimize == .Debug and (debug_ubsan or debug_asan) and !use_c and builtin.os.tag != .windows and builtin.zig_backend != .stage2_llvm) { // != .freestanding) {
    ...
    if (!use_c) {
        if (builtin.os.tag == .macos or builtin.os.tag == .linux) {
            cflags.append("-Wno-deprecated") catch {
                return;
            };
        }
    }
    ...
    //std.debug.print("Build type: {}\n", .{optimize});
    //if (use_c) {
    //    std.debug.print("Compiler  : {}\n", .{builtin.zig_exe});
    //} else {
    //    std.debug.print("Compiler  : {}\n", .{builtin.zig_exe});
    //}
